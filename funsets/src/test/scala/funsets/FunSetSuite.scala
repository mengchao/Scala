...
class FunSetSuite extends FunSuite {
  ...
  test("diff contains only former elements") {
    new TestSets {
      val s = diff(s1, s2)
  ...
  test("filter contains only the subset for which predicate holds") {
    new TestSets {
      val s = filter(s1, x => x <= 1)
      val t = filter(s2, x => x <= 1)
      assert(contains(s, 1), "Filter 1")
      assert(!contains(t, 2), "Filter 2")
    }
  }
    
  test("check whether all bounded integers within set satisfy certain predicate") {
    new TestSets {
      val s = union(s1, s2)
  ...
  test("check whether exists bounded integers within set satisfy certain predicate") {
    new TestSets {
      val s = union(s1, s2)
