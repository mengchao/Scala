...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    if (remainElems.isEmpty) {
      ...
      val mostRetweetedInRemaining = remainElems.mostRetweeted
      if(elem.retweets >= mostRetweetedInRemaining.retweets)
           elem
      else
...
object GoogleVsApple {
  ...
  def tweetsMentioning(dictionary: List[String]): TweetSet = {
    allTweets.filter(t => dictionary.exists(s => t.text.contains(s)))
  }
