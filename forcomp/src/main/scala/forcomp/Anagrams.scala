...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.groupBy((element: Char) => element.toLower).toList.map {
      case (char, charList) => (char, charList.length)
      }.sortBy {case (char, occur) => char}
  }
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = 
    dictionary.map(word => (word, wordOccurrences(word))).groupBy{case(word, occurs) => occurs}.map {
      case (occurs, words) => (occurs, words.map {case (w, occur) => w})}.withDefaultValue(List())
